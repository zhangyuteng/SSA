...
def build_link_map(text):
    ...
    if links:
        for link in map(extract_link_block, links):
            if not link:
                continue
...
if __name__ == '__main__':
    ...
    file_fomate = 'test/wiki_output_parsewiki_%02dParsed'
    posTAGRE = re.compile(r'[A-Z]\|[A-Z]')
    for i in range(1, 72):
        filename = file_fomate % i
        print "start reduce %s" % filename
        with open(filename, 'r') as f:
            saver = pickle.load(f)
        print 'the number of title is %s' % len(saver['title'])
        print 'the number of link is %s' % len(saver['link'])
        for link in saver['link']:
            pass
